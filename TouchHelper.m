...
{
    NSString * state= [ConfigService valueForName:@"TouchOpen"];
    return [kStrDef_Y isEqualToString:state];
...
{
    if (pLock) {
          [ConfigService editWithName:@"TouchOpen" value:kStrDef_Y];
    }else{
        [ConfigService editWithName:@"TouchOpen" value:kStrDef_N];
    }
...
+(void) checkTouchIdCallBack:(nullable void(^)(BOOL rState, NSError  * __nullable rError))pDoCallBack
{
    ...
    if ([context canEvaluatePolicy:LAPolicyDeviceOwnerAuthenticationWithBiometrics error:&error]) {
        ...
        [context evaluatePolicy:LAPolicyDeviceOwnerAuthenticationWithBiometrics localizedReason:@"请按home键指纹解锁" reply:^(BOOL success, NSError * _Nullable error) {
            if (success) {
                NSLog(@"验证成功 刷新主界面");
                pDoCallBack(YES,error);
            }else{
                pDoCallBack(NO,error);
